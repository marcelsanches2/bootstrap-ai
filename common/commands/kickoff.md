...
Initializes a project from scratch: collects requirements, generates a product brief, decides the stack, and routes to the correct preset.
...
**MANDATORY INTERACTIVE MODE — VIOLATION = BUG:**
- Your response must be ONLY question 1, nothing else. No additional text, no preamble.
- **STOP after each question.** Do not write anything else until the user answers.
- Only after the user's answer, confirm in 1 line and ask the next question.
- **NEVER** ask 2+ questions in the same message.
- **NEVER** proceed to Phase 2 without the user confirming all answers.
...
After all 7 answers, validate with the user:
...
Based on the answers (especially questions 5 and 6), decide the stack:
...
| Full-stack web (monolith) | Next.js / Remix / Nuxt / SvelteKit | `fullstack-web` |
...
2. /jarvis-plan — generate plan with embedded perspectives (manual)
3. Implement plan
...
- Do not propose technical solutions during clarify. Redirect: "Let's finish the requirements first."
- If the user can't answer, mark as `[OPEN QUESTION: ...]` — do not make it up.
