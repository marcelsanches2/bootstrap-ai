---
name: jarvis-plan
description: Unified planning — explores codebase, grills if necessary, generates technical plan with all perspectives built in for Python backend. One LLM pass.
---
...
Do not execute implementation. Do not change production code. Only plan.
...
**When the grill is finished, generate summary in table format:**
...
| Service, repository, model, use case, Python layer | `product_roles/role-backend-architect.md` |
...
Consult the `## Blocking rules` section of the guides referenced by the selected roles (step 3). The plan MUST NOT be proposed if it violates any rule listed as blocking in those guides.
...
4. Do not continue to implementation. Do not generate code. Do nothing else until the user responds.
