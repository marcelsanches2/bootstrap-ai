...
2. **If the answer is in the codebase, search instead of asking.** Explore models, services, routes, configurations — anywhere that resolves the doubt without human interaction.
3. **Each question comes with:**
   - Clear recommendation
   - Why (technical or product reason)
   - Viable alternatives (if any)
4. **Maximum 7 questions.** If you need more, the task is too big — suggest breaking it into smaller tasks.
...
- **Compare with what exists**: "CartModel doesn't have a discount field. Add to existing model or create a separate entity?"
...
- **Propose ADRs** when the decision meets all 3 criteria:
  ...
  2. Surprising — future reader will wonder "why did they do it this way?"
  3. Real trade-off — there were genuine alternatives with specific reasons

  If it meets all 3, suggest creating `docs/adr/<NNNN>-<slug>.md` with 1-3 sentences.
