---
name: jarvis-plan
description: Unified planning — explores codebase, grills if necessary, generates technical plan with all perspectives embedded for fullstack React web + Node.js backend. One LLM pass.
---
...
- Explore the relevant codebase (components, hooks, routes, services, models, existing tests)
...
| Testable frontend flow, integration, state, data fetching, web business logic | `product_roles/role-web-qa.md` |
| Endpoint, HTTP contract, status code, schema, OpenAPI | `product_roles/review-api.md` |
| Testable backend flow, usecase, API business logic, integration | `product_roles/role-api-qa.md` |
