---
name: jarvis-plan
description: Planejamento unificado — explora codebase, grilla se necessário, gera plano técnico com todas as perspectivas embutidas para fullstack React web + Node.js backend. Um pass de LLM.
---
...
- Explore o codebase relevante (componentes, hooks, rotas, serviços, modelos, testes existentes)
...
{Seção gerada pelo architect + frontend architect + node architect — componentes, hooks, estado, rotas, módulos, serviços, transações, tipos}

## {Seções de domain reviews conforme roles selecionadas (Acessibilidade, Performance, API, Database, Security, Observability, Scalability)}

## Testes

{Seção gerada pelo Web QA + API QA}
